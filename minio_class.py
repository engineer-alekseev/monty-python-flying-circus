...
class Minio_client:
    ...
    def get_url(self,filename:str)->str:
        ...
        Returns:
            str: The presigned URL for the object.
        Raises:
            None.
        """
        # Get the presigned URL for the object
        if self.object_exists(filename):
            url:str = self.client.presigned_get_object(self.bucket,
                                                     filename, 
    ...
    def put_file(self,filename:str,content:str)->None:
        ...
        if not self.object_exists(filename):
            self.client.fput_object(self.bucket,filename,content)
        else:
            logging.warning("Object exists: %s", filename)
    def get_all_objects(self):
        objects = self.client.list_objects(self.bucket, recursive=True)
        return list(objects)
# if __name__ == "__main__":
#     m = Minio_client('config.ini')
#     print(m.get_all_objects())
